...
func (tfb *TfServiceDefinitionV1) ToService(tfBinContext executor.TFBinariesContext) (*broker.ServiceDefinition, error) {
	...
	return &broker.ServiceDefinition{
		...
		ProviderBuilder: func(logger lager.Logger, store broker.ServiceProviderStorage) broker.ServiceProvider {
			...
			return NewTerraformProvider(NewTfJobRunner(
				store,
				tfBinContext,
				workspace.NewWorkspaceFactory(),
				invoker.NewTerraformInvokerFactory(executorFactory, tfBinContext.Dir, tfBinContext.ProviderReplacements),
			), logger, constDefn, store, NewLastOperationStore(store))
		},
	}, nil
}
